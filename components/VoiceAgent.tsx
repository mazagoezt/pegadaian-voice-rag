      ...
      try {
        ...
        if (msg.type === "response.required_action" && msg.response?.required_action?.type === "submit_tool_outputs") {
          ...
          for (const c of calls) if (c.type === "function" && c.name === "search_company") {
            ...
            if (speakAnswers) { const speak: AnyObj = { type:"response.create", response:{ modalities:["audio"], instructions:"Bacakan jawaban di atas secara ringkas." } }; dc.send(JSON.stringify(speak)); }
          }
