...
export default function VoiceAgent() {
  ...
  async function connect() {
    // Siapkan elemen audio (autoplay) dalam konteks gesture
    try {
    ...
    try {
      ...
      setStatus("Izin mikrofon ditolak/tidak tersedia: " + (e?.message || String(e)));
      return;
    }
    ...
    // Pastikan siap menerima audio dari server
    try {
    ...
    pc.ontrack = (event) => {
      ...
      if (el) {
        ...
        el
          .play()
          .then(() => pushLog("Audio playing"))
          .catch((err) => pushLog("Audio play blocked: " + (err?.message || String(err))));
      }
    };

    // Kirim audio mic ke server
    ms.getTracks().forEach((t) => pc.addTrack(t, ms));

    // DataChannel untuk event Realtime
    const dc = pc.createDataChannel("oai-events");
    dcRef.current = dc;

    dc.onopen = () => {
      setStatus("Terhubung — mendaftar tools...");
      // Konfigurasi sesi: paksa panggil tool, pakai VAD server, suara shimmer
      const sessionUpdate: AnyObj = {
        type: "session.update",
        session: {
          instructions:
            "Kamu asisten Pegadaian. Bahasa Indonesia natural, ramah. Jawab HANYA berdasarkan fungsi search_company; jangan sebutkan sumber/URL.",
          ...
          tools: [
            {
              ...
              name: "search_company",
              description:
                "Ringkas jawaban dari domain resmi berdasarkan query (tanpa URL).",
              parameters: {
                type: "object",
    ...
    dc.onmessage = async (e) => {
      try {
        ...
        if (msg.type && typeof msg.type === "string" && msg.type.startsWith("response.")) {
          pushLog("evt:" + msg.type);
        }

        // Alur tool-calling Realtime: required_action -> submit_tool_outputs
        if (
          msg.type === "response.required_action" &&
          msg.response?.required_action?.type === "submit_tool_outputs"
        ) {
          ...
          for (const c of calls) {
            if (c.type === "function" && c.name === "search_company") {
              ...
              const out: AnyObj = {
                ...
                tool_outputs: [
                  { tool_call_id: c.id, output: res?.answer || res?.text || "" },
                ],
              };
              ...
              if (speakAnswers) {
                const ev: AnyObj = {
                  ...
                  response: {
                    instructions: "Silakan bacakan jawaban di atas secara ringkas.",
                    modalities: ["audio"],
                  },
                };
  ...
  // Tombol “Tes Audio” -> paksa model berbicara; “Beep” -> test speaker lokal
  function speakTest() {
    const dc = dcRef.current;
    if (!dc || dc.readyState !== "open") return;
    const ev = {
      type: "response.create",
      response: { instructions: "Katakan: Tes audio berhasil.", modalities: ["audio"] },
    };
    dc.send(JSON.stringify(ev));
  }
  function beepTest() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        {!connected ? (
          <button onClick={connect} className="px-4 py-2 rounded-2xl bg-emerald-600 text-white">
            Hubungkan &amp; Bicara
          </button>
        ) : (
          <button onClick={disconnect} className="px-4 py-2 rounded-2xl bg-slate-700 text-white">
            Putuskan
          </button>
        )}
      ...
      <div className="flex items-center gap-3">
        <button onClick={speakTest} className="px-2 py-1 rounded bg-sky-600 text-white text-xs">
          Tes Audio
        </button>
        <button onClick={beepTest} className="px-2 py-1 rounded bg-amber-600 text-white text-xs">
          Beep
        </button>
