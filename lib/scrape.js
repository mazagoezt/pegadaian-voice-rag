...
export function scrapeWithFallback(html) {
  const $ = cheerio.load(html);
  const title = ($("meta[property='og:title']").attr("content") || $("title").text() || "").trim();
  let textBlocks = [];
  const pick = (sel) => ($(sel).text() || "").replace(/\s+/g, " ").trim();
  textBlocks.push(pick("main")); textBlocks.push(pick("article")); textBlocks.push(pick("body"));
  textBlocks.push(pick("noscript")); textBlocks.push(pick("h1, h2, h3, h4, h5, h6")); textBlocks.push(pick("p, li"));
