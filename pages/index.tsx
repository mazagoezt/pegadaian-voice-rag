...
export const getServerSideProps = async () => {
  return { props: {} }; // Force SSR, disable SSG
};

export default function Home() {
  return (
    <main className="max-w-2xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-bold">Pegadaian Voice RAG — Vercel (v3.8.3c)</h1>
      ...
      <section className="p-4 rounded-2xl border bg-white">
        ...
        <VoiceAgent />
        <div className="mt-2 text-xs text-slate-500">Jika suara tak keluar, coba <b>Tes Audio (REST)</b> atau lihat <b>Debug</b>.</div>
      </section>
      ...
      <section className="p-4 rounded-2xl border bg-white">
        ...
        <form onSubmit={async (e)=>{
          ...
          (document.getElementById("ans") as any).textContent = j?.answer || j?.text || "Tidak ada jawaban.";
        }} className="space-y-2">
          <div className="flex gap-2">
            <input name="q" placeholder="Tulis pertanyaan..." className="flex-1 px-3 py-2 rounded-xl border" />
            <button className="px-3 py-2 rounded-xl bg-indigo-600 text-white">Tanya</button>
          </div>
        </form>
      ...
      <section className="p-4 rounded-2xl border bg-white">
        ...
        <form onSubmit={async(e)=>{
          ...
          const j = await res.json();
          (document.getElementById("idx") as any).textContent = j.ok ? "Sukses: dokumen="+j.docs : "Gagal: "+j.error;
        }}>
