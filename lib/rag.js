...
function splitToChunks(text, size = CHUNK_SIZE, overlap = CHUNK_OVERLAP) {
  const clean = (text || "").replace(/\s+/g, " ").trim();
  if (!clean) return []; const out = [];
  for (let i = 0; i < clean.length; i += Math.max(1, (size - overlap))) { out.push(clean.slice(i, i + size)); if (i + size >= clean.length) break; }
  return out.slice(0, 40);
}

async function embedChunk(inputs) {
  ...
  const j = JSON.parse(text); return j.data.map((d) => d.embedding);
}
...
function pushFees(url, title, text) {
...
async function discoverLinksFromHome(domain) {
  try {
    const res = await fetch(domain, { headers: { "User-Agent": BROWSER_UA }, cache: "no-store" });
    if (!res.ok) return []; const html = await res.text(); const $ = cheerio.load(html); const origin = new URL(domain).origin; const hrefs = new Set();
...
async function fetchSitemapUrls(domain) {
  const urls = new Set();
  if (!ONLY_EXTRA) {
    try { const r = await fetch(new URL("/sitemap.xml", domain).toString(), { headers: { "User-Agent": BROWSER_UA }, cache: "no-store" }); if (r.ok) { const xml = await r.text(); const locs = [...xml.matchAll(/<loc>(.*?)<\/loc>/g)].map(m => m[1]); for (const u of locs) if (/(pegadaian\.co\.id)/.test(new URL(u).hostname)) urls.add(u); } } catch {}
...
async function fetchPageText(url) {
  const html = await fetch(url, { headers: { "User-Agent": BROWSER_UA }, cache: "no-store" }).then(r => r.text());
  const { title, content } = scrapeWithFallback(html);
  const $ = cheerio.load(html);
  $("table").each((_, t) => { const rows = []; $(t).find("tr").each((_, tr) => { const cells = $(tr).find("th,td").map((i, td) => $(td).text().trim()).get(); if (cells.length) rows.push(cells.join(" | ")); }); if (rows.length) pushFees(url, title || url, rows.join(". ")); });
...
export function searchFees(query) {
