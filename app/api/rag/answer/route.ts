...
import { NextRequest, NextResponse } from "next/server"; import { searchRag, searchFees } from "@/lib/rag"; import { bestCanonicalUrl } from "@/lib/canonical"; import { scrapeWithFallback } from "@/lib/scrape";
...
function isDef(q: string){ return /(apa itu|apa yang dimaksud|jelaskan|definisi)/i.test(q); } function isFee(q: string){ return /(biaya|tarif|sewa modal|bunga|margin|ujrah|administrasi|denda)/i.test(q); }
async function fetchText(u: string){ const html = await fetch(u, { headers: { "User-Agent": BROWSER_UA }, cache: "no-store" as any }).then(r => r.text()); return scrapeWithFallback(html); }
export async function POST(req: NextRequest){ const body = await req.json(); const query = String(body?.query || ""); if(!query) return NextResponse.json({ error:"query required"},{status:400});
  ...
  } catch(e:any){ return NextResponse.json({ answer:"Maaf, terjadi kendala.", error:e?.message || String(e) }, { status:500 }); }
}
