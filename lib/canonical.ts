...
export function bestCanonicalUrl(query: string, extraCsv: string): string | null {
  ...
  let best: string | null = null; let bestScore = 0; const qSlug = qTokens.join("-");
  for (const u of urls) {
    let score = 0; const slug = tokens(u).join("-");
    for (const t of qTokens) if (slug.includes(t)) score += 1;
    const boosts: [RegExp, number][] = [[/pinjaman-serbaguna/,3],[/gadai-emas/,3],[/tabungan-emas/,3],[/kur-syariah/,2],[/gadai-elektronik/,2],[/gadai-kendaraan/,2]];
